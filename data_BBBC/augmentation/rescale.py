...
class Rescale(DataAugment):
    ...
    def apply_rescale(self, image, label, sf_x, sf_y, random_state):
        ...
        y_length = int(sf_y * image.shape[0])
        if y_length <= image.shape[0]:
            y0 = random_state.randint(low=0, high=image.shape[0]-y_length+1)
            y1 = y0 + y_length
            transformed_image = transformed_image[y0:y1, :]
            transformed_label = transformed_label[y0:y1, :]
        else:
            y0 = int(np.floor((y_length - image.shape[0]) / 2))
            y1 = int(np.ceil((y_length - image.shape[0]) / 2))
            transformed_image = np.pad(transformed_image, ((y0, y1),(0, 0)), mode='constant')
            transformed_label = np.pad(transformed_label, ((y0, y1),(0, 0)), mode='constant')

        x_length = int(sf_x * image.shape[1])
        if x_length <= image.shape[1]:
            x0 = random_state.randint(low=0, high=image.shape[1]-x_length+1)
            x1 = x0 + x_length
            transformed_image = transformed_image[:, x0:x1]
            transformed_label = transformed_label[:, x0:x1]
        else:
            x0 = int(np.floor((x_length - image.shape[1]) / 2))
            x1 = int(np.ceil((x_length - image.shape[1]) / 2))
            transformed_image = np.pad(transformed_image, ((0, 0),(x0, x1)), mode='constant')
            transformed_label = np.pad(transformed_label, ((0, 0),(x0, x1)), mode='constant')
