...
class Elastic(DataAugment):
    ...
    def __call__(self, data, random_state=np.random):
        ...
        transformed_image = cv2.remap(image, mapx, mapy, self.image_interpolation, borderMode=self.border_mode)
        if 'label' in data and data['label'] is not None:
            transformed_label = cv2.remap(label, mapx, mapy, self.label_interpolation, borderMode=self.border_mode)
        else:
