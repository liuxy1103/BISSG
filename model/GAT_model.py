...
class GATLayerAdj(nn.Module):
    ...
    def __init__(self, d_i, d_o, act=F.relu, eps=1e-6):
        ...
        self.w = nn.Linear(2 * d_i, 1)  # weights for edges
        self.act = act
        self._init_weights()
    ...
    def forward(self, x, adj, src, tgt, Msrc, Mtgt):
        ...
        """
        N = x.size()[0]
        hsrc = x.unsqueeze(0).expand(N, -1, -1)  # 1,N,i  (N,N,i) different in column
        htgt = x.unsqueeze(1).expand(-1, N, -1)  # N,1,i  (N,N,i) different in row
    ...
    def __init__(self, d_in, d_out, num_heads):
        ...
        self.GAT_heads = nn.ModuleList(
            [
                ...
                for _ in range(num_heads)  #initial parameters is different
            ]
        )
...
class GAT_MNIST(nn.Module):
    ...
    def __init__(self, num_features, num_classes, num_heads=[2, 2, 2]):
        ...
        self.layer_heads = [1] + num_heads #[1,2,2,2]
        ...
        self.GAT_layers = nn.ModuleList(
            [
                ...
                self.GAT_layer_sizes[1:], #cascaded的multi_GAT
                self.layer_heads[:-1],
                self.layer_heads[1:],
            )
            ]
